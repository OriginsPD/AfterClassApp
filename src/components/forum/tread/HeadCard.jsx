import { TagIcon } from "@heroicons/react/outline";
import parse from "html-react-parser";
import PingLoader from "../../loaders/PingLoader";

const HeadCard = ({ discussionState }) => {
	// console.log(discussionState);
	return (
		<>
			{discussionState.map((items) => (
				<div
					key={items.id}
					className="mx-auto max-w-2xl rounded-lg bg-white px-8 py-4 shadow-md "
				>
					<div className="flex items-center justify-between">
						<span className="text-sm font-light text-gray-600 ">
							{new Date(items.created_at).toLocaleDateString("Jamaica", {
								month: "long",
								day: "2-digit",
								year: "2-digit",
							})}
						</span>

						<div className="transform cursor-pointer rounded bg-blue-600 px-3 py-1 text-sm font-bold text-gray-100 transition-colors duration-200 hover:bg-gray-500">
							{items.topic.name}
						</div>
					</div>

					<div className="mt-2">
						<div className="text-2xl font-bold text-gray-700 hover:text-gray-600  ">
							{items.name}
						</div>
						<p className="mt-2 text-gray-600 ">{parse(items.content)}</p>
					</div>

					<div className="my-4 grid grid-cols-3 gap-2">
						{items.discussion_tags.map((innerVal) => (
							<div
								key={innerVal.id}
								className="leading-sm group ml-4 inline-flex w-full items-center rounded-full border bg-white px-1 py-1  text-xs font-bold uppercase text-gray-700 hover:bg-blue-600 hover:text-white"
							>
								<TagIcon className="mr-2 h-4 w-4 pl-1 group-hover:animate-pulse " />
								{innerVal.tag.name}
							</div>
						))}
					</div>

					<div className="mt-4 flex items-center justify-end">
						<div className="flex items-center">
							<img
								className="mx-4 hidden h-10 w-10 rounded-full object-cover sm:block"
								src={
									items.user.username
										? `https://ui-avatars.com/api/?background=random&color=fff&name=${items.user.username}`
										: " data:image/jpeg;base64,"
								}
								alt="avatar"
							/>
							<a className="cursor-pointer font-bold text-gray-700 ">
								{items.user.username}
							</a>
						</div>
					</div>
				</div>
			))}
			{Object.keys(discussionState).length == 0 ? <PingLoader /> : null}
		</>
	);
};

export default HeadCard;
